pipeline{
    ...
    stages{
        ...
        stage('get-keypair'){
            ...
            steps{
                sh '''
                    if [ -f "lera_cw_public.pem" ]
                    then 
                        echo "file exists..."
                    else
                        aws ec2 create-key-pair \
                          --region us-east-1 \
                          --key-name lera_cw.pem \
                          --query KeyMaterial \
                          --output text > lera_cw.pem

                        chmod 400 lera_cw.pem

                        ssh-keygen -y -f lera_cw.pem >> lera_cw_public.pem
                    fi
                '''                
            }
        }
                    ...
                    then
                        ...
                        then
                            eksctl create cluster \
                                ...
                                --nodes-max 2 \
                                --ssh-access \
                                --ssh-public-key  lera_cw_public.pem \
                                --managed
                        else
